...
- =:q= : Defines a question. Transpiles to a level-1 heading (~*~) in Org-mode. =:penalty= is the percentage deducted if =:forbidden= symbols are detected in the student's code.
...
#+begin_src lisp
  CL-USER> (cg:gen-exam-files "~/tmp/assessment1.sxm" :include-hidden t)
  ;; Output:
...
During the practicum, students solve the programming problems defined in the assessment. While the *PDF* serves as their primary guide, the students interact with the CodeGrader system in the Lisp REPL through a specialized function called ~CHK-MY-SOLUTION~ designed to ensure compliance and promote better Lisp programming habits.
...
The ~CHK-MY-SOLUTION~ functio is more than a simple tester; it is a "live" assistant that helps students address issues before the final evaluation.
...
3. *Style Coaching (via lisp-critic)*: Runs the student’s source code through the lisp-critic library to identify non-idiomatic patterns or "code smells." It translates these automated critiques into immediate feedback, suggesting more "Lisp-y" ways to structure their logic.
...
While your code may be functional, the suggestions below help you
write more "Lisp-y" solutions.
**********************************************************************
