...
1. The instructor writes the assessment in s-expression markdown (SXM).
...
Use CodeGrader’s GEN-EXAM-FILES function (with your /<SXM assessment file>/) to generate the following files files at *./Gen-files/*:
- an orgmode file containing de assessment description -- you can then use emacs to export this file to any format you want (htmo, pdf, etc.)
- a *tooling (.data) file* containing assessment data CodeGrader needs to autograde students' solutions. 

For example: When executed in the REPL, the form (GEN-EXAM-FILES "[[https://raw.githubusercontent.com/marcus3santos/CodeGrader/refs/heads/main/Example/pt.sxm][pt.sxm]]") will generate [[https://github.com/marcus3santos/CodeGrader/tree/main/Example/Gen-files][these files]] in the *./Gen-files*
folder.
