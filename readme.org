...
- *Step 4: Evaluation (Final Grading)* :: 
  Once the exam concludes and solutions are collected on the host computer, the instructor uses CodeGrader to batch-process the submissions. The system evaluates each solution against the hidden assertions stored in the *.data* file to generate objective final grades.
...
;; --- Atomic Types ---
<string>            ::= "characters"
<symbol>            ::= keyword-or-lisp-symbol
<integer>           ::= [0-9]+
<number>            ::= [0-9]+ | [0-9]+.[0-9]+
<list>              ::= (<atom>*)
<lisp-form>         ::= any-valid-common-lisp-form
<lisp-code>         ::= any-valid-common-lisp-program-text
#+end_example
...
#+begin_src lisp
  ...
  Assessment Org-mode file generated: ~/tmp/assessment1.org
  Assessment HTML file generated: ~/tmp/assessment1.html
  Assessment PDF ile generated: ~/tmp/assessment1.pdf
  Assessment metadata file generated: ~/tmp/assessment1.pdf
  SXM>
#+end_src
...
- The PDF and HTML files generated from the Org-mode file.
- A data file (=~/tmp/Gen-files/assessment1.data=) that CodeGrader utilizes for the automated grading of student solutions.
*NOTE*: To omit the hidden test cases in the data file, call GEN-EXAM-FILES setting :INCLUDE-HIDDEN to NIL.
