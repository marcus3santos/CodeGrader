...
<doc-node>         ::= "(" ":doc" <doc-props> <doc-content-list> ")"
<doc-props>        ::= "(" ":title" <string> ":folder" <string> ")"

<doc-content-list> ::= <standard-node> | <standard-node> <doc-content-list>

<standard-node>    ::= <string> 
                     | <p-node> | <ul-node> | <ol-node> | <li-node> 
                     | <s-node> | <cb-node> | <wa-node> | <q-node>

<q-node>           ::= "(" ":q" <q-props> <q-content-list> ")"
<q-props>          ::= "(" [ ":title" <string> ] 
                           [ ":forbidden" <list> ":penalty" <number> ] ")"

<q-content-list>   ::= <q-element> | <q-element> <q-content-list>
<q-element>        ::= <string> | <p-node> | <ul-node> | <ol-node> 
                     | <li-node> | <s-node> | <cb-node> | <wa-node>
                     | <tc-node> | <sols-node>

<tc-node>          ::= "(" ":tc" <tc-props> <tc-content-list> ")"
<tc-props>         ::= "(" ":function" <symbol> ")"

<tc-content-list>  ::= <tc-element> | <tc-element> <tc-content-list>
<tc-element>       ::= <string> | <p-node> | <cb-node> | <gvn-node> | <hdn-node>

<gvn-node>         ::= "(" ":gvn" <data-content-list> ")"
<hdn-node>         ::= "(" ":hdn" <data-content-list> ")"

<data-content-list> ::= <data-element> | <data-element> <data-content-list>
<data-element>      ::= <string> | <p-node> | <a-node>

<a-node>            ::= "(" ":a" <lisp-expression> <lisp-expression> ")"

<sols-node>        ::= "(" ":sols" <sols-list> ")"
<sols-list>        ::= <sol-node> | <sol-node> <sols-list>
<sol-node>         ::= "(" ":sol" <lisp-program> ")"

<p-node>           ::= "(" ":p" <general-list> ")"
<ul-node>          ::= "(" ":ul" <general-list> ")"
<ol-node>          ::= "(" ":ol" <general-list> ")"
<li-node>          ::= "(" ":li" <general-list> ")"
<s-node>           ::= "(" ":s" <s-props> <general-list> ")"
<wa-node>          ::= "(" ":wa" <general-list> ")"
<cb-node>          ::= "(" ":cb" <cb-props> <general-list> ")"

<general-list>     ::= <standard-node> | <standard-node> <general-list>

<s-props>          ::= "(" [ ":level" <integer> ] [ ":title" <string> ] ")"
<cb-props>         ::= "(" ":language" <string> ")"

<string>           ::= "\"" <characters> "\""
<symbol>           ::= <alphanumeric-sequence>
<integer>          ::= <digit> | <digit> <integer>
<number>           ::= <integer> | <integer> "." <integer>
<list>             ::= "(" <element-list> ")"
<lisp-expression>  ::= <any-valid-common-lisp-form>
<lisp-program>     ::= <any-valid-common-lisp-code>
#+end_example
...
(:doc ;; The assessment description
  ...
  (:q ;; a question description; transated as a level 1 section "*" in orgmode
    ...
    (:wa  ;; What students are asked; must appear within a :q expression
      ...
      (:sols
        (:sol
          (defun count-occurrences (a b &optional (acc 0))
            (cond ((null b) acc)
                  ((= a (car b)) (count-occurrences a (cdr b) (1+ acc)))
                  (t (count-occurrences a (cdr b) acc)))))
        (:sol
          (defun count-occurrences (a b)
            (cond ((null b) 0)
                  ((= a (first b)) (1+ (count-occurrences a (rest b))))
                  (t (count-occurrences a (rest b)))))))  
  (:q ;; another question...))
#+end_example
...
- =:q= — A section describing an individual exercise or problem.
  - =:title= — The question title. The default title is `"Question".
...
- =(:sols (:sol ... ...) (:sol ... ...)= - Node containing the question solution(s). The :sol node contains the forms of a solution. 
