...
- In the Linux test environment, computer IDs are is the computer

We assume there is zip archive file containing the students' solutions originally stored in specific folders named
*cps305YY/engXXX-ZZ/pt/* or *cps305YY/otest-KK*, where *YY* is the course section number and *KK* is an integer number,
