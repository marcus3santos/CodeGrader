...
(defun replace-vars-in-bdy (ll bdy env)
  (let* ((ll-symbs (get-symbols ll))
         (new-ll (mapcar #'(lambda (x)
                             (change-vars x ll-symbs))
                         ll)))
    (values new-ll
            (if (listp bdy)
                (mapcar #'(lambda (x)
                            (change-vars x (append ll-symbs env)))
                        bdy)
                (change-vars bdy (append ll-symbs env))))))

(defun change-vars (f &optional symbs-codes )
        ...
        ((listp f)
         (let ((op (car f)))
           (cond
             ((or (and (eql op 'defun)   ; DEFUN, LAMBDA
              ...
              (let* ((lambda-list (if (eql op 'defun)
                     ...
                     (bdy (if (eql op 'defun)
                              (cdddr f)
                              (cddr f))))
                (multiple-value-bind (new-ll new-bdy) (replace-vars-in-bdy lambda-list bdy symbs-codes)
                  (if (eql op 'defun)
              ...
              (let ((defs (second f))
                ...
                `(,op ,(mapcar (lambda (def)
                                 (let* ((fname (first def))
                                   ...
                                   (multiple-value-bind (new-ll new-bdy) (replace-vars-in-bdy lambda-list (cddr def) symbs-codes)
                                     `(,fname ,new-ll
                                              ,@new-bdy))))
                               defs)
                      ,@(mapcar (lambda (x)
                                  (change-vars x symbs-codes))
                                bdy))))
             ((and (eq op 'multiple-value-bind) ; MULTIPLE-VALUE-BIND
                   (listp (second f))
                   (third f))
              (let* ((lambda-list (variables-in-lambda-list (cadr f))))
                (multiple-value-bind (new-ll new-bdy) (replace-vars-in-bdy lambda-list (cdddr f) symbs-codes)
                  `(multiple-value-bind ,new-ll ,(change-vars (caddr f) symbs-codes)
                     ,@new-bdy))))
              ...
              (let* ((lvars (if (eql op 'dotimes)
                ...
                (multiple-value-bind (_ new-bdy)
                  (if (or (eq op 'do) (eq op 'do*))
                      (replace-vars-in-bdy lvars (cdddr f) symbs-codes)
                      (replace-vars-in-bdy lvars (cddr f) symbs-codes))
                  (declare (ignore _))
                  `(,op
                    ,(if (eql op 'dotimes)
                         (mapcar (lambda (x)
                                   (multiple-value-bind (_ new-exp) (replace-vars-in-bdy lvars x symbs-codes)
                                         (declare (ignore _))
                                         new-exp))
                                 (second f))
                         (mapcar (lambda (x)
                                   (if (listp x) ; takes care of bindings of the kind ((x 1) y (z 2) ...)
                                       `(,(multiple-value-bind (_ new-var) (replace-vars-in-bdy lvars (car x) symbs-codes)
                                            (declare (ignore _))
                                            new-var)
                                         ,@(multiple-value-bind (_ new-exp) (replace-vars-in-bdy lvars (cdr x) symbs-codes)
                                             (declare (ignore _))
                                             new-exp))
                                       `(,(multiple-value-bind (_ new-var) (replace-vars-in-bdy lvars x symbs-codes)
                                            (declare (ignore _))
                                            new-var) nil)))
                                 (second f)))
             ...
             (t (append  (list (car f))
                         (mapcar #'(lambda (f)
                                     (change-vars f symbs-codes))
                                 (cdr f)))))))
        (t f)))


(defun normalize (f)
  (let ((macro-expanded (expand-macros f))
